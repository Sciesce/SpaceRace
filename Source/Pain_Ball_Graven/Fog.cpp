...
AFog::AFog() : m_wholeTextureRegion(0,0,0,0, m_textureSize, m_textureSize)
{
	...
	RootComponent = m_squarePlane; //setting as root component
	m_squarePlane->SetCollisionProfileName(UCollisionProfile::NoCollision_ProfileName); //setting collision
	{
	...
	m_squarePlane->TranslucencySortPriority = 100; //sort order to 100 to  cast over lesser objects

	m_squarePlane->SetRelativeScale3D(FVector(m_coverSize, m_coverSize, 1)); //setting plane to m_coverSize x m_coverSize

	// Load the base material from your created material.  
	{
		static ConstructorHelpers::FObjectFinder <UMaterial> asset(TEXT("Material'/Game/Shader/FogMat.FogMat'")); //had to ensure "Shader" folder was named correctly
